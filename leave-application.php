...
$sql = mysqli_query($db, "SELECT company_name FROM tbl_companies WHERE ID = '$company_id'");
                ...
                <div class="row">
                    <div class="col-md-5">
                        <div class="form-group">
                            ...
                            <select name="delegate_emp_number" id="delegate_name" required class="select-chosen"
                                data-placeholder="Choose a requestor..." style="width: 250px;">
                                <option></option>
                                <?php
                                if ($_SESSION['hris_role'] == 'User') {
                                    if (allowed_on_behalf_filing($emp_num) == '0') {
                                        ...
                                        WHERE t1.company = '$company_name' AND t.super_admin != '1'");
                                        while ($row = mysqli_fetch_assoc($get_employees)) {
                                            echo '<option value="' . $row['employee_number'] . '">' . $row['account_name'] . '</option>';
                                        }
                                    }
                                } elseif ($_SESSION['hris_role'] == 'Admin') {
                                    $get_employees = mysqli_query($db, "SELECT t.*, t1.company 
                                    FROM tbl_personal_information t
                                    INNER JOIN tbl_employment_information t1
                                    ON t.employee_number = t1.employee_number
                                    WHERE t.super_admin != '1'");
                                    while ($row = mysqli_fetch_assoc($get_employees)) {
                                        echo '<option value="' . $row['employee_number'] . '">' . $row['account_name'] . '</option>';
                                    }
                                } else {
                    ...
                    <div class="col-md-5">
                        <div class="form-group">
                            ...
                            <select name="leave_type" id="leave_type" required class="select-chosen"
                                data-placeholder="Choose a Leave Type..." style="width: 250px;">
                                <option></option>
                ...
                <div class="row">
                    <div class="col-md-5">
                        <div class="form-group">
                            ...
                            <select name="duration" id="duration" required class="select-chosen"
                                data-placeholder="Choose a Leave Type..." style="width: 250px;">
                                <option></option>
                    ...
                    <div class="col-md-2">
                        <div class="form-group">
                            ...
                            <input type="text" name="approver" id="approver" readonly class="form-control"
                                value="<?= $approver ?>">
                        </div>
                    </div>
                </div>
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if (data === "") {
                            ...
                            $('#res').html(
                                "Note: The following dates were already filed: " +
                                data);
                            $('#btn_leave_application').prop("disabled", true);
                        }
                    }
...
$('#leave_type').on('change', function() {
    ...
    var available_leaves = ['SL', 'VL',
        'others'
    ]; // MNCS = maternity normal or cs -- MM = Maternity Miscarriage 
